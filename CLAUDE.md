...
#### Issue Funnel (Воронка замечаний → WP-debt) — v2 DB-native

> **Двухуровневый триаж (R7 Триажёр):**
> - **Auto-triage (Grade 1):** при helpful=false / ✏️ comment → Haiku classify → `feedback_triage` DB → TG alert if severity>=high
> - **Review (Grade 3):** при открытии сессии техдолга → structured report + 2 файловых intake
>
> **Источники данных:**
> - `feedback_triage` DB ← auto-classify при каждом событии (бот, `core/feedback_triage.py`)
> - `unsatisfied-questions.md` ← **REPORT** из feedback_triage (daily, unsatisfied-report.sh). НЕ inbox
> - `fleeting-notes.md` ← заметки пользователя из TG (`.баг ...`)
> - `captures.md` ← Note-Review (знание из заметок), Session-Close KE, пользователь (вручную)
>
> **Review-протокол (при открытии сессии техдолга):**
> 1. Прочитать unsatisfied-questions.md (structured report: ✏️ → urgent → кластеры)
> 2. Прочитать 2 файловых intake (fleeting-notes, captures)
> 3. Прочитать WP-debt backlog
> 4. Review предклассифицированных + оценить бюджет
> 5. Одобренные → WP-debt backlog, отработанные → `status='resolved'`
>
...
> *«**Роль:** [из каталога ролей, DP.AGENT.001 §3.2]. **Работа:** [что]. **Рабочий продукт:** [артефакт]. **Метод:** [как]. **Оценка времени:** [~Xh / ~Xмин].»*

> Если роли нет в каталоге — объявить: *«Роль не найдена в каталоге. Предлагаю добавить: [название роли].»* Зафиксировать пробел для дополнения DP.AGENT.001.
...
### Протокол Work: Pre-action Gates (блокирующее)

> **Перед необратимым действием — проверь соответствующий протокол.** Правила ниже существуют в других секциях, но пропускаются, когда не привязаны к конкретному моменту. Эта секция привязывает их к моментам.

| Момент | Проверка | Источник правила |
|--------|---------|-----------------|
| **Перед `git commit`** в репо с CLAUDE.md | Прочитать CLAUDE.md репо → выполнить exit protocol (если есть) ДО push | checklists.md «Перед изменением репозитория» |
| **Перед предложением решения** с архитектурным выбором | АрхГейт (§5): таблица ЭМОГСС **в первом** ответе, не после критики | CLAUDE.md §5 |

> **Почему здесь, а не только в §5?** АрхГейт описан в §5, exit protocol — в CLAUDE.md репо. Оба пропускаются по одной причине: правило есть, но не привязано к конкретному действию в потоке работы. Эта таблица — точка привязки.
...
7. **WP Context File:** Если РП in_progress и ≥2 сессий → обновить/создать `DS-strategy/inbox/WP-{N}-{slug}.md`. Если РП done → обновить фронтматтер WP-файла: `status: done` (или `merged` / `drop`). **Не перемещать** в archive — это обязанность роли R1 Стратег, сценарий Session-Prep (Пн 4:00).
...
| Session-Prep (подготовка к сессии) | Стратег (launchd, Пн 4:00) | strategist-agent/prompts/session-prep.md |
| Strategy-Session (сессия стратегирования) | Стратег (вручную, после session-prep) | strategist-agent/prompts/strategy-session.md |
| Day-Plan | Стратег (launchd, Вт-Вс 4:00) | strategist-agent/prompts/day-plan.md |
| Note-Review (разбор заметок) | Стратег (launchd, 23:00 / ручной) | strategist-agent/prompts/note-review.md |
| Day-Close | Стратег (ручной, по запросу) | strategist-agent/prompts/day-close.md |
| Week-Review | Стратег (launchd, Пн 00:00) | strategist-agent/prompts/week-review.md |
