...
2. Заменяет 7 плейсхолдеров (`gleniboutique`, `/c/Users/pc/Github` и др.)
...
1. Замените `gleniboutique` на ваш GitHub username во всех файлах
2. Замените `/c/Users/pc/Github` на путь к рабочей директории (напр. `~/Github`)
3. Замените `/c/Users/pc` на домашнюю директорию (значение `$HOME`)
4. Замените `-c-Users-pc-Github` на путь через дефисы (напр. для `~/Github` → `-Users-yourname-Github`)
5. Замените `6` на час запуска стратега в UTC (напр. `4` для 7:00 MSK)
6. Замените `7:00 CET` на описание времени (напр. `7:00 MSK`)
7. Замените `/c/Users/pc/AppData/Roaming/npm/claude` на путь к Claude CLI (напр. `/opt/homebrew/bin/claude`)
