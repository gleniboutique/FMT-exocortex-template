...
find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while read file; do
    sed -i \
        -e "s|gleniboutique|$GITHUB_USER|g" \
        -e "s|/c/Users/pc/Github|$WORKSPACE_DIR|g" \
        -e "s|/c/Users/pc/AppData/Roaming/npm/claude|$CLAUDE_PATH|g" \
        -e "s|-c-Users-pc-Github|$CLAUDE_PROJECT_SLUG|g" \
        -e "s|6|$TIMEZONE_HOUR|g" \
        -e "s|7:00 CET|$TIMEZONE_DESC|g" \
        -e "s|/c/Users/pc|$HOME_DIR|g" \
        "$file"
done
